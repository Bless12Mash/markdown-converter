...
export class MarkdownProcessor {
    ...
    private createHeadingParagraph(token: TokenType): Paragraph {
        return new Paragraph({
            ...
            heading: MarkdownProcessor.headingLevelMap[token.depth as keyof typeof MarkdownProcessor.headingLevelMap],
                ...
                case 'blockquote':
                    if (token.tokens) {
                        token.tokens.forEach(quoteToken => {
                            paragraphs.push(this.createBlockQuoteParagraph(quoteToken.text || ''));
                        });
                    }
                    break;
